...
#    #end def
    ...
    @classmethod
    def fetch_queries(cls, email, queries, from_dbs=None, **kwargs):
        if not email:
        ...
        with ProgressCounter('', 0, replace=False) as prg:
            ...
            for db in batch:
                if not from_dbs or db in from_dbs:
                    fetched += be.get_records_for_terms([q.term for q in batch[db]], db)
                    prg.count()
        return fetched
    ...
    def fetch_results(cls, email, results, from_dbs=None, what='record', **kwargs):
        if what not in cls._fetch_targets:
            raise ValueError('"what" parameter may only be one of: %s' % str(cls._fetch_targets))
        queries = []
        for ali in cls.iter_alignments(results):
            q = cls.Query(ali, what)
            if q: queries.append(q)
        return cls.fetch_queries(email, queries, from_dbs, **kwargs)
#end class
